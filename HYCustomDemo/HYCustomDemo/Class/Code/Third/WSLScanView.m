...
- (void)drawRect:(CGRect)rect{
    ...
    self.myCode = [[UIButton alloc] initWithFrame:CGRectMake(0, self.scanRetangleRect.origin.y + self.scanRetangleRect.size.height + 10 + 20 + 10, rect.size.width, 20)];
    self.myCode.titleLabel.font = [UIFont systemFontOfSize:15];
    [self.myCode setTitle:@"识别二维码" forState:UIControlStateNormal];
    [self.myCode setTitleColor:[UIColor greenColor] forState:UIControlStateNormal];
    [self.myCode addTarget:self action:@selector(myCodeClicked) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:self.myCode];
